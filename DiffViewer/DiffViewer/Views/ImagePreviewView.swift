...
struct ImagePreviewView: View {
    ...
    var body: some View {
        HStack(spacing: 0) {
            if file.changeType == .deleted {
                ...
                // modified or renamed: show old and new side by side
    ...
    private var newImage: NSImage? {
        let path = (repoPath as NSString).appendingPathComponent(file.fileName)
        return NSImage(contentsOfFile: path)
    }

    private var oldImage: NSImage? {
        // Get image from git HEAD
        let data = GitService.showFileData(fileName: file.fileName, repoPath: repoPath)
